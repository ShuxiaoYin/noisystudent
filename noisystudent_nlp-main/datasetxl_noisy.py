...
class DatasetXL(Dataset):
    ...
    def _read_noisy(self,manifest):
        with open(manifest,'r',encoding='utf8') as f:
            for line in f.readlines():
                info = json.loads(line.strip())
                tmp = self._tokenizer._tokenize(info['noisy_src'])
                info['len'] = len(tmp)
                self._data[int(info["index"])] = info
...
def customize_collate_fn(data_batch):
    src_texts,noisy_texts, texts,inds,texts_ori = zip(*data_batch)
    src = TOKENIZER_bert(src_texts, truncation=True, padding=True)
    noisy_src = TOKENIZER_bert(noisy_texts,truncation=True, padding=True)
    return src, noisy_src,torch.tensor(texts), torch.tensor(inds),torch.tensor(texts_ori)
